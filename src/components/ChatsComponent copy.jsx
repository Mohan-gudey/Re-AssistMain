  ...
  const handleFileSelect = async (e) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        console.log('✅ Processed:', { abstract, keywords });
      } catch (err) {
    ...
    setDroppedFiles([]); // Reset droppedFiles if used
    setProcessedPapers(processed); // Store processed info per file
    setLoading(false);
  };
  ...
  const handleUploadFiles = async () => {
    ...
    try {
      for (const paper of processedPapers) {
        ...
        // If you're uploading the file to S3 via backend:
        formData.append("files", paper.file); // `files` matches multer field
  
        // If you're uploading via a URL (optional):
        if (paper.link) {
          formData.append("url", paper.link);
        }
  
        // You can add abstract/keywords too, but make sure backend handles them!
        ...
        const paperResult = await paperResponse.json();
        const paperId = paperResult.papers[0]._id; // ✅ Use the first paper
        ...
        await fetch(`https://re-assist-backend.onrender.com/api/projects/${projectId}/add-paper`, {
          ...
          body: JSON.stringify({ paperId }), // ✅ correct paperId
        });
